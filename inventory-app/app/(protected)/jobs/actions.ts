...
const safeError = (message: string) => { const code = message.match(/(?:^|: )([A-Z][A-Z0-9_]+)$/)?.[1] ?? ''; const known = new Set(['ACCESS_DENIED','JOB_VERSION_CONFLICT','INVALID_JOB_TRANSITION','JOB_NOT_EDITABLE','JOB_ALREADY_COMPLETED','PRICE_PENDING','CUSTOMER_ARCHIVED','VEHICLE_CUSTOMER_MISMATCH','PRODUCT_INACTIVE','INSUFFICIENT_STOCK','RESERVATION_INCONSISTENT','IDEMPOTENCY_KEY_REUSED']); return known.has(code) ? code.replaceAll('_', ' ').toLowerCase().replace(/^./, c => c.toUpperCase()) : 'The workshop job change could not be saved.'; };
...
export async function transitionJobAction(jobId: string, version: number, status: string) { const access = await getCurrentAccess(); if (!hasPermission(access, 'jobs.edit')) throw new Error('You do not have permission to edit jobs.'); const { error } = await (await createServerSupabaseClient()).rpc('transition_job', { p_job_id: jobId, p_expected_version: version, p_status: status }); if (error) throw new Error(safeError(error.message)); revalidatePath('/jobs'); revalidatePath(`/jobs/${jobId}`); }
export async function cancelJobAction(jobId: string, version: number) { const access = await getCurrentAccess(); if (!hasPermission(access, 'jobs.edit')) throw new Error('You do not have permission to edit jobs.'); const { error } = await (await createServerSupabaseClient()).rpc('cancel_job', { p_job_id: jobId, p_expected_version: version }); if (error) throw new Error(safeError(error.message)); revalidatePath('/jobs'); revalidatePath(`/jobs/${jobId}`); }
export async function completeJobAction(jobId: string, version: number) { const access = await getCurrentAccess(); if (!hasPermission(access, 'jobs.complete')) throw new Error('You do not have permission to complete jobs.'); const { error } = await (await createServerSupabaseClient()).rpc('complete_job', { p_job_id: jobId, p_expected_version: version, p_request_id: randomUUID() }); if (error) throw new Error(safeError(error.message)); revalidatePath('/jobs'); revalidatePath(`/jobs/${jobId}`); }
