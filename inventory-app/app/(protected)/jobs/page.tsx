...
export default async function JobsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (!hasPermission(access, 'jobs.view')) return <div className="operations-page"><PageHeader title="Jobs" subtitle="Permission denied" /></div>;
  ...
  return (
    <div className="operations-page max-w-6xl">
      ...
      <form className="flex flex-wrap gap-3 rounded-xl border bg-card p-4" role="search">
        ...
        <button className="h-10 rounded-md bg-secondary px-4 text-sm font-medium">Search</button>
      </form>
      {error ? (
        <div className="mt-5 rounded-xl border border-destructive/40 p-5 text-sm">Jobs could not be loaded.</div>
      ) : result.rows.length === 0 ? (
        <div className="mt-5 rounded-xl border p-8 text-sm text-muted-foreground">
          {cursor ? 'No more jobs match this view.' : 'No jobs match this view.'}
        </div>
      ) : (
        <div className="mt-5 grid gap-3">
          {result.rows.map((row) => (
            <Link key={String(row.id)} href={`/jobs/${row.id}`} className="rounded-xl border bg-card p-4 hover:border-primary">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <span className="font-semibold">{String(row.job_number)}</span>
                <StatusBadge status={String(row.status)}>{String(row.status).replaceAll('_', ' ')}</StatusBadge>
              </div>
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                <span>{String(row.customer_name ?? 'Walk-in')}</span>
                <span>{String(row.vehicle_registration ?? 'No vehicle')}</span>
                <span>{row.pricing_complete ? `$${Number(row.total_incl_gst ?? 0).toFixed(2)} incl GST` : 'PRICE PENDING'}</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      {!error && (cursor || result.hasMore) ? (
        <nav aria-label="Job pages" className="mt-5 flex items-center justify-between text-sm">
          {cursor ? <Link className="rounded-md border px-3 py-2" href={href(status, query, null)}>Back to first page</Link> : <span />}
          {result.hasMore && result.nextCursor ? (
            <Link className="rounded-md border px-3 py-2" href={href(status, query, result.nextCursor)}>Next</Link>
          ) : null}
        </nav>
      ) : null}
    </div>
