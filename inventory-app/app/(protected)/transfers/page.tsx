...
export default async function TransfersPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  return <div className="grid gap-6">
    <PageHeader title="Stock transfers" subtitle="Move stock between Regency Park and AWT Tyres Website with a complete audit trail." actions={hasPermission(access, 'inventory.transfer_request') ? <Link className={buttonVariants()} href="/transfers/new">New transfer</Link> : undefined} />
