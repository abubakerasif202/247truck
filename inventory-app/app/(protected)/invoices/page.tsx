...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <div className="operations-page max-w-6xl">
    ...
    {!result.ok ? (
      ...
      <div className="rounded-xl border p-8 text-sm text-muted-foreground">No invoices match these filters.</div>
    ) : (
      <div className="grid gap-3">{result.rows.map((row) => { const status = displayStatus(row); const isOverduePartial = row.display_status === 'overdue' && row.payment_state === 'partial'; return <Link key={row.id} href={`/invoices/${row.id}`} className="rounded-xl border bg-card p-4 hover:border-primary focus-visible:outline-2 focus-visible:outline-offset-2"><div className="flex flex-wrap items-center justify-between gap-3"><span className="font-semibold">{row.invoice_number}</span><StatusBadge status={status}>{status}{isOverduePartial ? ' · partially paid' : ''}</StatusBadge></div><p className="mt-2 break-words text-sm text-muted-foreground">{row.customer_name ?? 'Walk-In Customer'} · {row.source_type}{row.due_date ? ` · due ${row.due_date}` : ''} · {row.pricing_complete ? `$${Number(row.total_incl_gst ?? 0).toFixed(2)}` : 'PRICE PENDING'}{row.balance != null ? ` · $${Number(row.balance).toFixed(2)} due` : ''}</p></Link>; })}</div>
    )}
    {result.ok ? (() => { const pages = Math.max(Math.ceil(result.total / result.limit), 1); return <nav aria-label="Invoice pages" className="mt-5 flex items-center justify-between text-sm"><span>Page {Math.min(page,pages)} of {pages} · {result.total} invoices</span><div className="flex gap-2">{page > 1 ? <Link className="rounded-md border px-3 py-2" href={href(params,{page:String(page-1)})}>Previous</Link> : null}{page < pages ? <Link className="rounded-md border px-3 py-2" href={href(params,{page:String(page+1)})}>Next</Link> : null}</div></nav>; })() : null}
  </div>;
}
