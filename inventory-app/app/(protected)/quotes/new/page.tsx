...
export default async function NewQuotePage() {
  ...
  const client = await createServerSupabaseClient(); const scope = await getCurrentLocationScope(access); const [customers, products, locations] = await Promise.all([listSalesCustomers(client), listSalesProducts(client, scope), client.from('locations').select('id,code').eq('active', true).order('code')]);
