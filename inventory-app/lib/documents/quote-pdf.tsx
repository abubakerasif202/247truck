...
function Line({ line }: { line: QuoteDocumentLine }) { const detail = [line.tyre ? [line.tyre.brand, line.tyre.model, line.tyre.size].filter(Boolean).join(' · ') : '', line.torqueNm && Number(line.torqueNm) > 0 ? `Torque: ${line.torqueNm} Nm` : ''].filter(Boolean).join(' · '); return <View style={styles.description}><Text style={styles.main}>{line.description}</Text>{detail ? <Text style={styles.detail}>{detail}</Text> : null}</View>; }
...
export function QuotePdfDocument({ quote, logoSource }: { quote: QuoteDocumentData; logoSource?: string | null }) {
  ...
  // React-PDF otherwise inserts render-time metadata, breaking durable retry fingerprints.
  const dateOnly = quote.quoteDate?.match(/^\d{4}-\d{2}-\d{2}/)?.[0] ?? '1970-01-01';
  const parsedCreatedAt = new Date(`${dateOnly}T00:00:00.000Z`);
  const createdAt = Number.isNaN(parsedCreatedAt.getTime()) ? new Date('1970-01-01T00:00:00.000Z') : parsedCreatedAt;
  return <Document title={`Quote ${quote.quoteNumber}`} author={business} subject="Customer quote" creationDate={createdAt} modificationDate={createdAt}>
    <Page size="A4" style={styles.page} wrap>
      <View style={styles.header} fixed><View>{logoSource ? <Image src={logoSource} style={quote.business.logo_asset_path === '/brand/logo-real-horizontal.png' ? styles.truckLogo : styles.logo} /> : <Text style={styles.brand}>{business}</Text>}</View><View><Text style={styles.title}>QUOTE</Text><Text style={styles.number}>#{quote.quoteNumber}</Text></View></View>
      ...
      {quote.extraDescription || quote.customerNotes ? <View style={styles.serviceDetails}><Text style={styles.label}>SERVICE DETAILS / NOTES</Text>{quote.extraDescription ? <View style={styles.serviceRow}><Text style={styles.dateLabel}>Extra Description</Text><Text>{quote.extraDescription}</Text></View> : null}{quote.customerNotes ? <View style={styles.serviceRow}><Text style={styles.dateLabel}>Notes</Text><Text>{quote.customerNotes}</Text></View> : null}</View> : null}{quote.business.invoice_footer || quote.branch.document_footer ? <View style={styles.notes}><Text>{quote.business.invoice_footer ?? quote.branch.document_footer}</Text></View> : null}
