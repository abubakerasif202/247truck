...
const styles = StyleSheet.create({
  ...
  suppliedTruckLogo: { width: 150, height: 90, objectFit: 'cover', objectPosition: 'center center' },
...
export function InvoicePdfDocument({ invoice, logoSource }: { invoice: InvoiceDocumentData; logoSource?: string | null }) {
  ...
  const primary = invoice.business.primary_colour ?? RED;
  const vehicleFields = [
  ...
  return <Document title={`Tax Invoice ${invoice.invoiceNumber}`} author={businessName} subject="Tax invoice" creationDate={issuedAt} modificationDate={issuedAt}>
    <Page size="A4" style={styles.page} wrap>
      <View style={[styles.header, { borderBottomColor: primary }]} fixed>
        ...
        <View>{logoSource ? <Image src={logoSource} style={invoice.business.logo_asset_path === '/brand/logo-247-invoice-2026.png' ? styles.suppliedTruckLogo : invoice.business.logo_asset_path === '/brand/logo-real-horizontal.png' ? styles.truckLogo : styles.logo} /> : <Text style={[styles.brandName, { color: primary }]}>{businessName}</Text>}</View>
      ...
      <View style={styles.metaGrid}>
        <View style={styles.metaColumn}><Text style={styles.label}>From</Text><Text style={styles.partyName}>{businessName}</Text>{invoice.business.abn ? <Text style={styles.line}>ABN {invoice.business.abn}</Text> : null}{addressLines(invoice.business).map((item) => <Text key={item} style={styles.line}>{item}</Text>)}{invoice.business.phone ?? invoice.branch.phone ? <Text style={styles.line}>{invoice.business.phone ?? invoice.branch.phone}</Text> : null}{invoice.business.shared_email ?? invoice.branch.contact_email ? <Text style={styles.line}>{invoice.business.shared_email ?? invoice.branch.contact_email}</Text> : null}{invoice.business.website ? <Text style={styles.line}>{invoice.business.website}</Text> : null}</View>
