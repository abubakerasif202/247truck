...
const optionalNullableText = (max: number) => z.union([trimmed.max(max), z.null(), z.literal('')]).transform((value) => value === '' ? null : value).optional();
...
const optionalTorque = z.union([z.number(), z.string(), z.null(), z.literal('')]).transform((value) => value === '' ? null : value == null ? null : String(value)).pipe(z.union([z.string().regex(/^\d+(?:\.\d{1,2})?$/).refine((value) => Number.isFinite(Number(value)) && Number(value) > 0, 'Torque must be greater than zero.'), z.null()])).optional();
...
const header = {
  ...
  issue_date: date, due_date: date, customer_reference: optionalText(200), customer_notes: optionalText(2000), extra_description: optionalNullableText(5000), internal_notes: optionalText(5000),
