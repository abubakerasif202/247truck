...
export async function listSalesProducts(client: SupabaseClient, scope: LocationScope, query = '', locationId: string | null = null) {
  ...
  return [...unique.values()].map(row => ({ productId: String(row.product_id), name: String(row.name), brandName: row.brand_name ? String(row.brand_name) : null, patternName: row.pattern_name ? String(row.pattern_name) : null, sizeName: row.size_name ? String(row.size_name) : null, tyreCondition: row.tyre_condition === 'used' || row.tyre_condition === 'new' ? row.tyre_condition : null, retailPriceInclGst: row.retail_price_incl_gst == null ? null : Number(row.retail_price_incl_gst), wholesalePriceInclGst: row.wholesale_price_incl_gst == null ? null : Number(row.wholesale_price_incl_gst), sellingPriceInclGst: row.selling_price_incl_gst == null ? null : Number(row.selling_price_incl_gst), available: Number(row.available ?? 0), onHand: Number(row.on_hand ?? 0), reserved: Number(row.reserved ?? 0) }));
}
