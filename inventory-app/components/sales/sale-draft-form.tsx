...
type Props = { action: (formData: FormData) => void | Promise<void>; locationId?: string; actionLabel?: string; initialCustomerId?: string; initialCustomer?: Customer | null; initialVehicleId?: string; initialVehicle?: Vehicle | null; initialLines?: Line[]; allowWalkIn?: boolean; tenderMode?: boolean };
...
export function SaleDraftForm({ action, locationId = '', actionLabel = 'Save draft', initialCustomerId = '', initialCustomer = null, initialVehicleId = '', initialVehicle = null, initialLines = [], allowWalkIn = false, tenderMode = false }: Props) {
  ...
  const [requestId] = useState(() => typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : '');
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { getJson<{ products: Product[] }>(`/api/sales/products?q=${encodeURIComponent(productQuery)}&location_id=${encodeURIComponent(locationId)}&pricing_tier=${tier}`, controller.signal).then(result => { if (sequence === productSequence.current) setProductResults(result.products); }).catch(error => { if (error.name !== 'AbortError') setSearchError('Product search is temporarily unavailable.'); }); }, 250);
    return () => { window.clearTimeout(timer); controller.abort(); };
  }, [productQuery, locationId, customer]);
  ...
  const addProduct = () => {
    ...
    setLines([...lines, { line_type: 'product', product_id: selectedProduct.productId, used_tyre_unit_id: isUsed ? usedUnitId : undefined, description: `${selectedProduct.name}${isUsed ? ` · ${usedUnits.find(unit => unit.id === usedUnitId)?.internal_unit_code ?? 'used unit'}` : ''}`, quantity: parsedQuantity, unit_price_incl_gst: selectedPrice }]);
  ...
  return <form action={action} className="grid gap-5 rounded-xl border bg-card p-5">
    <input type="hidden" name="request_id" value={requestId} /><input type="hidden" name="location_id" value={locationId} /><input type="hidden" name="customer_id" value={customer?.id ?? initialCustomerId} /><input type="hidden" name="customer_vehicle_id" value={vehicleId} /><input type="hidden" name="lines" value={JSON.stringify(lines)} /><input type="hidden" name="tenders" value={JSON.stringify(tenders)} /><input type="hidden" name="walk_in_name" value={walkInName} /><input type="hidden" name="walk_in_phone" value={walkInPhone} /><input type="hidden" name="walk_in_email" value={walkInEmail} />
