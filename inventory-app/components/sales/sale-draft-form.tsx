...
type Props = { action: (formData: FormData) => void | Promise<void>; locationId?: string; locations?: LocationOption[]; requestId: string; actionLabel?: string; initialCustomerId?: string; initialCustomer?: Customer | null; initialVehicleId?: string; initialVehicle?: Vehicle | null; initialLines?: Line[]; allowWalkIn?: boolean; tenderMode?: boolean };
...
export function SaleDraftForm({ action, locationId = '', locations = [], requestId, actionLabel = 'Save draft', initialCustomerId = '', initialCustomer = null, initialVehicleId = '', initialVehicle = null, initialLines = [], allowWalkIn = false, tenderMode = false }: Props) {
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    const timer = window.setTimeout(() => { getJson<{ products: Product[] }>(`/api/sales/products?q=${encodeURIComponent(productQuery)}&location_id=${encodeURIComponent(selectedLocationId)}`, controller.signal).then(result => { if (sequence === productSequence.current) setProductResults(result.products); }).catch(error => { if (error.name !== 'AbortError') setSearchError('Product search is temporarily unavailable.'); }); }, 250);
    return () => { window.clearTimeout(timer); controller.abort(); };
  }, [productQuery, selectedLocationId]);
  ...
  return <form action={action} className="grid gap-5 rounded-xl border bg-card p-5">
    <input type="hidden" name="request_id" value={requestId} /><input type="hidden" name="location_id" value={selectedLocationId} /><input type="hidden" name="customer_id" value={customer?.id ?? initialCustomerId} /><input type="hidden" name="customer_vehicle_id" value={vehicleId} /><input type="hidden" name="lines" value={JSON.stringify(lines)} /><input type="hidden" name="tenders" value={JSON.stringify(tenders)} />
    {!hasFixedLocation ? <div className="grid gap-2"><Label htmlFor="location_picker">Branch</Label><select id="location_picker" aria-label="Branch" value={selectedLocationId} onChange={event => setSelectedLocationId(event.target.value)} className="h-11 rounded-md border border-input bg-background px-3" required><option value="">Select branch</option>{locations.map(loc => <option key={loc.id} value={loc.id}>{LOCATION_NAMES[loc.code as LocationCode] ?? loc.code}</option>)}</select>{!selectedLocationId ? <p role="alert" className="text-xs text-destructive">Select the branch this sale is for before adding lines.</p> : null}</div> : null}
    ...
    <SaleSubmitButton label={actionLabel} disabled={lines.length === 0 || !selectedLocationId} />
  </form>;
}
