...
export function SaleDraftForm({ customers, products, action, locationId = '', actionLabel = 'Save draft' }: { customers: Customer[]; products: Product[]; action: (formData: FormData) => void; locationId?: string; actionLabel?: string }) {
  ...
  return <form action={action} className="grid gap-5 rounded-xl border bg-card p-5">
    ...
    <div className="grid gap-2"><Label htmlFor="customer_id">Customer</Label><select id="customer_id" name="customer_id" required className="h-11 rounded-md border border-input bg-background px-3"><option value="">Select customer</option>{customers.map(c => <option key={c.id} value={c.id}>{c.customerNumber} · {c.displayName}{c.phone ? ` · ${c.phone}` : ''}</option>)}</select></div>
    <div className="grid gap-2"><Label htmlFor="customer_vehicle_id">Vehicle ID (optional)</Label><Input id="customer_vehicle_id" name="customer_vehicle_id" placeholder="Select a vehicle from the customer record" /></div>
