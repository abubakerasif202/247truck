...
export function SaleDraftForm({ customers, products, action, locationId = '', actionLabel = 'Save draft', initialCustomerId = '', initialVehicleId = '', initialLines = [], allowWalkIn = false }: { customers: Customer[]; products: Product[]; action: (formData: FormData) => void; locationId?: string; actionLabel?: string; initialCustomerId?: string; initialVehicleId?: string; initialLines?: Line[]; allowWalkIn?: boolean }) {
  ...
  return <form action={action} className="grid gap-5 rounded-xl border bg-card p-5">
    ...
    <div className="grid gap-2"><Label htmlFor="customer_id">Customer</Label><select id="customer_id" name="customer_id" defaultValue={initialCustomerId} required={!allowWalkIn} className="h-11 rounded-md border border-input bg-background px-3">{allowWalkIn ? <option value="">Walk-in customer</option> : <option value="">Select customer</option>}{customers.map(c => <option key={c.id} value={c.id}>{c.customerNumber} · {c.displayName}{c.phone ? ` · ${c.phone}` : ''}</option>)}</select>{allowWalkIn ? <input type="hidden" name="walk_in_label" value="Walk-in customer" /> : null}</div>
