...
export function TransferRequestForm({ locations, products, managerLocationId }: { locations: Location[]; products: Product[]; managerLocationId: string | null }) {
  ...
  const defaultSource = own ?? locations.find(location => location.code === 'REG') ?? locations[0];
  const other = locations.find(location => location.id !== (defaultSource?.id ?? managerLocationId));
  return <form action={action} className="grid gap-5 rounded-xl border bg-card p-5">
    <div className="grid gap-2"><Label htmlFor="source">Source branch</Label><select id="source" name="source_location_id" required defaultValue={defaultSource?.id} className="h-10 rounded-md border bg-background px-3">{locations.map(location => <option key={location.id} value={location.id}>{location.name}{managerLocationId && location.id !== managerLocationId ? ' (request from branch)' : ''}</option>)}</select></div>
