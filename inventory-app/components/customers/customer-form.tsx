...
export function CustomerForm({action,requestId,customer}:Props){
  ...
  const [detailsOpen,setDetailsOpen]=useState(Boolean(customer));
  const hasOptionalErrors=Boolean(state?.fieldErrors && Object.keys(state.fieldErrors).some(field=>field!=='display_name'));
  ...
  return <form action={formAction} noValidate className="operations-panel grid gap-6 p-4 sm:p-6">
    ...
    <div className={fieldClass}><Label htmlFor="display_name">{type==='business'?'Display name':'Full name'}</Label><Input id="display_name" name="display_name" defaultValue={customer?.display_name??''} required/><ErrorText messages={state?.fieldErrors?.display_name}/></div>
    <details open={detailsOpen||hasOptionalErrors} onToggle={event=>setDetailsOpen(event.currentTarget.open)} className="rounded-lg border border-border p-4">
      <summary className="cursor-pointer text-sm font-semibold">Additional details (optional)</summary>
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
      {type==='individual'?<><div className={fieldClass}><Label htmlFor="first_name">First name</Label><Input id="first_name" name="first_name" defaultValue={customer?.first_name??''}/></div><div className={fieldClass}><Label htmlFor="last_name">Last name</Label><Input id="last_name" name="last_name" defaultValue={customer?.last_name??''}/></div></>:<><div className={fieldClass}><Label htmlFor="company_name">Company / trading name</Label><Input id="company_name" name="company_name" defaultValue={customer?.company_name??''}/></div><div className={fieldClass}><Label htmlFor="legal_name">Legal name</Label><Input id="legal_name" name="legal_name" defaultValue={customer?.legal_name??''}/></div><div className={fieldClass}><Label htmlFor="abn">ABN</Label><Input id="abn" name="abn" defaultValue={customer?.abn??''}/></div><div className={fieldClass}><Label htmlFor="phone">Primary phone</Label><Input id="phone" name="phone" type="tel" defaultValue={customer?.phone??''}/></div></>}
      {type==='individual'?<div className={fieldClass}><Label htmlFor="mobile">Mobile</Label><Input id="mobile" name="mobile" type="tel" defaultValue={customer?.mobile??''}/></div>:null}
      ...
      <div className={fieldClass}><Label htmlFor="suburb">Suburb</Label><Input id="suburb" name="suburb" defaultValue={customer?.suburb??''}/></div><div className={fieldClass}><Label htmlFor="state">State</Label><Input id="state" name="state" defaultValue={customer?.state??''}/></div><div className={fieldClass}><Label htmlFor="postcode">Postcode</Label><Input id="postcode" name="postcode" inputMode="numeric" defaultValue={customer?.postcode??''}/></div>
