...
export function InvoiceBrandSettingsForm({ settings }: { settings: InvoiceBrandPreview }) {
  ...
  return <form action={action} className="grid gap-4">
    ...
    <fieldset className="grid gap-4 rounded-md border p-3 sm:grid-cols-2"><legend>Address</legend>{['street_address','suburb','state','postcode','country'].map((key) => <Field key={key} name={`address.${key}`} label={key.replaceAll('_',' ')} initial={value(settings.address,key)}/>)}</fieldset>
    <fieldset className="grid gap-4 rounded-md border p-3 sm:grid-cols-2"><legend>Payment details</legend>{['bank_name','account_name','bsb','account_number','payment_reference','instructions'].map((key) => <Field key={key} name={`bank.${key}`} label={key.replaceAll('_',' ')} initial={value(settings.bank_instructions,key)}/>)}</fieldset>
    <div className="grid gap-1"><Label htmlFor={`footer-${settings.brand}`}>Invoice footer</Label><Textarea id={`footer-${settings.brand}`} name="invoice_footer" defaultValue={settings.invoice_footer ?? ''}/></div>
    {state && !state.ok ? <p role="alert" className="text-sm text-destructive">{state.error}</p> : null}{state?.ok ? <p role="status" className="text-sm text-emerald-700">Saved.</p> : null}<Button type="submit">Save {settings.business_name} brand</Button>
  </form>;
}
