...
test('Admin raises a manual service invoice, edits the draft and issues it', async ({ page }) => {
  ...
  await page.getByLabel('Description', { exact: true }).fill('Mobile callout and inspection');
  ...
  await page.getByLabel('Description', { exact: true }).fill('Mobile callout and inspection updated');
