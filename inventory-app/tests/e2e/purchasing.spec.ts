...
test('Admin and LON Manager complete the purchasing workflow without crossing branches', async ({ page }) => {
  ...
  await expect(locationSelect.locator('option')).toHaveText('LON — Adelaide Wholesale Tyres');
  ...
  await expect(page.getByText('Adelaide Wholesale Tyres (LON)', { exact: true })).toBeVisible();
