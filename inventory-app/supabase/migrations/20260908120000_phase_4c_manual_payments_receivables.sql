...
create function public.finalise_pos_sale(
...
declare
  ...
  payload jsonb; replay jsonb; job_result jsonb; completion jsonb; invoice_result jsonb;
  issued jsonb; payment_result jsonb; projection jsonb; result jsonb;
  job_id uuid:=p_job_id; job_version integer; invoice_id uuid; invoice_version integer;
  total numeric(14,2); tender_total numeric(14,2):=0; customer_type text:='walk_in';
  row jsonb; pos integer:=0; child uuid;
begin
  if actor is null or p_request_id is null or p_location_id is null then
    raise exception 'ACCESS_DENIED' using errcode='42501';
  end if;
  perform private.finance_guard('invoices.view',p_location_id);
  perform private.finance_guard('invoices.create',p_location_id);
  perform private.finance_guard('invoices.issue',p_location_id);
  if not (select private.sales_permission('jobs.view'))
    or not (select private.sales_permission('jobs.create'))
    or not (select private.sales_permission('jobs.edit'))
    or not (select private.sales_permission('jobs.complete'))
    or not (select private.sales_permission('pos.use')) then
    raise exception 'ACCESS_DENIED' using errcode='42501';
  end if;
  if p_tenders is not null and pg_catalog.jsonb_typeof(p_tenders)<>'array' then
    raise exception 'INVALID_PAYMENT_TENDERS' using errcode='22023';
  end if;
  if coalesce(pg_catalog.jsonb_array_length(p_tenders),0)>0 then
    perform private.finance_guard('payments.view',p_location_id);
    perform private.finance_guard('payments.record',p_location_id);
  end if;
  if p_customer_id is not null then
    customer_type:=null;
    select c.customer_type into customer_type from public.customers c
      where c.id=p_customer_id and c.active and (select private.sales_location_allowed(p_location_id));
    if customer_type is null then raise exception 'CUSTOMER_ARCHIVED' using errcode='22023'; end if;
  end if;
  if p_customer_id is null and (p_job->>'walk_in_label') is null then
    raise exception 'CUSTOMER_REQUIRED' using errcode='22023';
  end if;
  payload:=pg_catalog.jsonb_build_object(
    'location_id',p_location_id,'customer_id',p_customer_id,'customer_vehicle_id',p_customer_vehicle_id,
    'job_id',p_job_id,'expected_job_version',p_expected_job_version,'job',coalesce(p_job,'{}'::jsonb),
    'lines',coalesce(p_lines,'[]'::jsonb),'tenders',coalesce(p_tenders,'[]'::jsonb));
  replay:=private.finance_request(p_request_id,'finalise_pos_sale',payload);
  if replay is not null then return replay; end if;

  if job_id is null then
    child:=private.finance_child_uuid(p_request_id,'pos-create-job',1);
    job_result:=public.create_job(child,p_location_id,p_customer_id,p_customer_vehicle_id,
      coalesce(p_job,'{}'::jsonb)||jsonb_build_object('source_type','pos'),p_lines);
    job_id:=(job_result->>'job_id')::uuid;
    job_version:=(job_result->>'version')::integer;
  else
    if p_expected_job_version is null then raise exception 'JOB_VERSION_CONFLICT' using errcode='40001'; end if;
    perform 1 from public.jobs j where j.id=job_id and j.location_id=p_location_id
    ...
    job_result:=public.update_job(job_id,p_expected_job_version,coalesce(p_job,'{}'::jsonb),p_lines);
    job_version:=(job_result->>'version')::integer;
  end if;

  child:=private.finance_child_uuid(p_request_id,'pos-complete-job',1);
  completion:=public.complete_job(job_id,job_version,child);
  invoice_result:=private.finance_build_job_invoice(job_id);
  invoice_id:=(invoice_result->>'invoice_id')::uuid;
  child:=private.finance_child_uuid(p_request_id,'pos-issue-invoice',1);
  issued:=public.issue_invoice(child,invoice_id,(invoice_result->>'version')::integer);
  select r.total_incl_gst,i.version into total,invoice_version
    from public.invoices i join public.invoice_revisions r on r.id=i.current_revision_id
    where i.id=invoice_id for update;

  for row in select value from pg_catalog.jsonb_array_elements(coalesce(p_tenders,'[]'::jsonb)) loop
    pos:=pos+1;
    if row->>'amount' is null or row->>'amount' !~ '^[0-9]+([.][0-9]{1,2})?$'
      or (row->>'amount')::numeric<=0 then
      raise exception 'INVALID_PAYMENT_AMOUNT' using errcode='22023';
    end if;
    tender_total:=tender_total+(row->>'amount')::numeric;
  end loop;
  if customer_type<>'business' and tender_total<>coalesce(total,0) then
    raise exception 'FULL_SETTLEMENT_REQUIRED' using errcode='22023';
  end if;
  if coalesce(pg_catalog.jsonb_array_length(p_tenders),0)>0 then
    child:=private.finance_child_uuid(p_request_id,'pos-payment',1);
    payment_result:=public.record_invoice_payment(child,invoice_id,invoice_version,p_tenders);
  else
    projection:=private.finance_invoice_projection(invoice_id,true);
    payment_result:=projection-'payments';
  end if;
  projection:=private.finance_invoice_projection(invoice_id,true);
  result:=pg_catalog.jsonb_build_object(
    'request_id',p_request_id,'job_id',job_id,'job_version',(completion->>'version')::integer,
    'job_status','completed','invoice_id',invoice_id,'invoice_number',invoice_result->>'invoice_number',
    'status','issued','version',(select i.version from public.invoices i where i.id=invoice_id),
    'total_incl_gst',total,'payment',projection,'payment_result',payment_result);
  perform private.sales_audit('POS_SALE_FINALISED','invoice',invoice_id,p_location_id,
    pg_catalog.jsonb_build_object('request_id',p_request_id,'job_id',job_id,'tender_total',tender_total));
  perform private.finance_request_finish(p_request_id,'finalise_pos_sale',payload,p_location_id,invoice_id,result);
  return result;
end;
$$;

-- Replace the initial draft with the recovered POS implementation. It uses
