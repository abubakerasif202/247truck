-- Forward-only fix for optional-cost customer-return replays.
--
-- Customer returns may intentionally omit a unit cost. The authoritative
-- movement function stores the effective cost snapshot (the current WAC) in
-- inbound_unit_cost, so comparing that stored derived value with the caller's
-- omitted NULL incorrectly rejects a safe idempotent replay. The request
-- identity remains bound to every caller-controlled field; only this one
-- derived value is excluded when the movement type is customer_return and the
-- caller intentionally omits p_inbound_unit_cost.
...
begin
  ...
  if found then
    ...
    if v_existing.product_id <> p_product_id
      ...
      or (
        not (p_movement_type = 'customer_return' and p_inbound_unit_cost is null)
        and v_existing.inbound_unit_cost is distinct from
  ...
  perform set_config('app.inventory_movement_notes', '', true);
end;
$$;
