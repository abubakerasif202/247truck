...
create or replace function private.transfer_authorized(
  p_source uuid, p_destination uuid, p_write boolean default true
)
...
$$;
revoke execute on function private.transfer_authorized(uuid, uuid, boolean) from public, anon, authenticated, service_role;
...
declare v_actor uuid := (select auth.uid()); v_id uuid; v_number text; v_role text; v_line jsonb;
begin
...
declare v record; l record; b public.inventory_balances%rowtype; m uuid; result jsonb := '{}'::jsonb;
begin
  ...
  for l in select * from public.stock_transfer_lines where transfer_id=p_transfer_id order by id loop
    ...
    values(p_request_id,l.product_id,v.source_location_id,-l.approved_quantity,'transfer_out','Branch stock transfer','stock_transfer',p_transfer_id::text,null,b.weighted_average_cost,auth.uid(),p_transfer_id,l.id) returning id into m;
...
create or replace function private.replay_wac_at(p_product_id uuid, p_location_id uuid, p_until timestamptz default null)
returns numeric language plpgsql security definer set search_path = '' as $$
declare m record; q integer:=0; before_q integer; w numeric(14,4):=0; c numeric(14,4);
begin
  for m in select im.*, osa.unit_cost assigned_cost from public.inventory_movements im left join public.opening_stock_cost_assignments osa on osa.opening_movement_id=im.id where im.product_id=p_product_id and im.location_id=p_location_id and (p_until is null or im.created_at <= p_until) order by im.created_at,im.id loop
    ...
    if m.movement_type in ('quick_stock_in','used_unit_in','purchase_receipt','opening_stock','transfer_in') then
      c:=case when m.movement_type='opening_stock' then coalesce(m.inbound_unit_cost,m.assigned_cost) when m.movement_type='transfer_in' then (select private.replay_wac_at(m.product_id, tr.source_location_id, outm.created_at) from public.stock_transfer_lines tl join public.stock_transfers tr on tr.id=tl.transfer_id join public.inventory_movements outm on outm.transfer_line_id=tl.id and outm.movement_type='transfer_out' where tl.id=m.transfer_line_id) else m.inbound_unit_cost end;
...
$$;
revoke execute on function private.replay_wac_at(uuid,uuid,timestamptz) from public, anon, authenticated, service_role;

create or replace function private.rebuild_current_wac(p_product_id uuid,p_location_id uuid)
returns numeric language plpgsql security definer set search_path = '' as $$
declare v numeric; q integer;
begin
  select on_hand into q from public.inventory_balances where product_id=p_product_id and location_id=p_location_id for update;
  if not found then raise exception 'BALANCE_NOT_FOUND' using errcode='P0002'; end if;
  v:=private.replay_wac_at(p_product_id,p_location_id,null);
