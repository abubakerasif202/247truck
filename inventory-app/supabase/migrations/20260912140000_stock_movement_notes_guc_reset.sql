-- post_inventory_movement_with_notes sets a transaction-local GUC so the
-- capture_inventory_movement_notes trigger can attach the note to the next
-- inventory_movements insert. It never cleared that GUC afterward, so a
-- transaction that later called public.post_inventory_movement directly
-- (bypassing this wrapper) would have a stale note attached to an unrelated
-- movement row. No caller currently mixes both entry points in one
-- transaction, so this was not exploitable, but reset defensively as soon as
