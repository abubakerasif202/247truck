...
function Logo({ compact = false, white = false }: { compact?: boolean; white?: boolean }) {
  return (
    <Link className={`brand-logo${compact ? " brand-logo--compact" : ""}`} href="/" aria-label="24/7 Truck Tyre Services home">
      <Image src={white ? "/brand/logo-real-white.png" : "/brand/logo-real-horizontal.png"} alt="24/7 Truck Tyre Services" width={2172} height={724} priority={compact} sizes={compact ? "210px" : "260px"} />
    </Link>
  );
}

function PhoneCta({ className, overline }: { className: string; overline: string }) {
  return (
    <a className={className} href={PHONE_HREF}>
...
function Header() {
  ...
  useEffect(() => {
    if (!open) return;
    navigation.current?.querySelector<HTMLElement>(".nav-links a")?.focus();
    const background = [
    ...
    const handleMenuKeys = (event: KeyboardEvent) => {
      ...
      if (event.key === "Tab") {
        const focusable = [
          ...Array.from(navigation.current?.querySelectorAll<HTMLElement>("a[href], button") ?? []),
      ...
      <div className="header-inner">
        ...
        <nav ref={navigation} id="primary-navigation" className={`main-nav${open ? " is-open" : ""}`} aria-label="Primary navigation" aria-hidden={isMobile && !open} inert={isMobile && !open ? true : undefined}>
          <p className="drawer-title">
            <span>Menu</span>
            <button type="button" className="drawer-close" aria-label="Close navigation" onClick={closeMenu}>×</button>
          </p>
          ...
          <div className="drawer-foot">
            <a href={PHONE_HREF}><small>24 hours · 7 days</small><span>Call {PHONE_DISPLAY}</span></a>
            <p>Regency Park, South Australia · roadside assistance across Adelaide.</p>
          </div>
        </nav>
        ...
        <div className="header-actions">
          ...
          <PhoneCta className="header-call" overline="24/7 assistance" />
    ...
    <footer className="site-footer">
      <div className="footer-grid">
        ...
        <div>
          ...
          <Link href="/fleet-roadside-assistance">National roadside program</Link>
...
function MobileActions() {
  return (
    <nav className="mobile-actions" aria-label="Quick contact actions">
      <a href={PHONE_HREF}><small>Call 24/7</small><span>{PHONE_DISPLAY}</span></a>
    ...
    <section className="hero" aria-labelledby="hero-title">
      ...
      <div className="hero-copy">
        ...
        <div className="hero-tertiary-row">
          <Link className="hero-tertiary" href="/book-wheel-alignment">Book wheel alignment ↗</Link>
          <Link className="hero-tertiary" href="/franchise">Franchise opportunities ↗</Link>
        </div>
      </div>
    </section>
  );
...
function TrustStrip() {
  const items = [
    ["◆", "Answering 24/7", "Urgent tyre help when the road does not wait."],
  ...
  return (
    <section className="section services-section" id="services">
      <div className="section-topline">
        <SectionHeading eyebrow="Our services" title="Complete truck services" intro="Roadside tyre support and practical workshop services built around the demands of working heavy vehicles." />
        ...
        {services.map((service) => (
          <Link className="service-card" href={service.href} key={`${service.number}-${service.title}`}>
            <span className="service-card-media">
              <Image className="service-card-image" src={service.image} alt={service.imageAlt} fill sizes="(max-width: 860px) 100vw, (max-width: 1180px) 50vw, 33vw" />
              <span className="service-number" aria-hidden="true">{service.number}</span>
            </span>
            <span className="service-card-body">
              <h3>{service.title}</h3>
              <p>{service.description}</p>
              <span className="card-link">Explore service <i aria-hidden="true">↗</i></span>
            </span>
          </Link>
        ))}
      </div>
  ...
  return (
    <section className="emergency-band">
      ...
      <div className="emergency-content">
        ...
        <h2>Broken down?<strong>We&apos;re ready 24/7.</strong></h2>
...
function WhyUs() {
  ...
  return (
    <section className="section why-section">
      <SectionHeading eyebrow="Why choose us" title="Reliable. Fast. Professional." intro="We understand tyre issues can stop your business. That is why the focus is on fast, practical and dependable truck tyre support." />
      <div className="why-layout">
        <div className="why-media">
          <Image src="/images/pack-04-wheel-fitting.webp" alt="Technician fitting a heavy commercial truck wheel" fill sizes="(max-width: 1180px) 420px, 30vw" />
          <span aria-hidden="true">24/7</span>
        </div>
        <ol>{reasons.map((reason, index) => <li key={reason}><span>{String(index + 1).padStart(2, "0")}</span><h3>{reason}</h3><i aria-hidden="true">↗</i></li>)}</ol>
      </div>
    </section>
  );
}
...
function ServiceAreas() {
  return (
    <section className="service-areas" id="service-areas">
      <div className="area-map" aria-hidden="true"><div className="sa-shape"><span>ADL</span><i /></div><p>South Australia<strong>Adelaide response zone</strong></p></div>
      <div className="area-content">
        <SectionHeading eyebrow="Where we operate" title="Adelaide, covered" intro="We provide truck tyre services across Adelaide and surrounding industrial and transport areas." />
        ...
        <a className="button button--red" href="#contact">Request assistance <span aria-hidden="true">↗</span></a>
      </div>
    </section>
  );
}
...
function EmergencyStrip() {
  return <section className="emergency-strip"><div><span className="phone-ring" aria-hidden="true">☎</span><div><h2>Truck tyre emergency? We&apos;re ready 24/7</h2><p>Call now for fast truck tyre assistance across Adelaide.</p></div></div><a href={PHONE_HREF}><small>Call now</small>{PHONE_DISPLAY}</a></section>;
}
  ...
  return (
    <section className={`section gallery-section${full ? " gallery-section--full" : ""}`}>
      ...
      <div className="gallery-grid">
        {items.map((item, index) => { const [src, label, alt] = item; return <figure key={src} className={`gallery-item gallery-item--${index + 1}`}><button type="button" onClick={(event) => { galleryOpener.current = event.currentTarget; setSelected(item); }} aria-label={`Open ${label} image`}><Image src={src} alt={alt} fill sizes={full ? "(max-width: 520px) 100vw, (max-width: 860px) 50vw, 33vw" : "(max-width: 520px) 100vw, (max-width: 1180px) 50vw, 25vw"} /><figcaption><span>{label}</span><small>View image ↗</small></figcaption></button></figure>; })}
      </div>
...
function InstagramSection() {
  return (
    <section className="instagram-section">
      ...
      <div className="instagram-copy"><p className="eyebrow"><span />24/7 Truck Tyre Services</p><h2>Follow us on Instagram</h2><p>See truck tyre work, roadside callouts and updates from 24/7 Truck Tyre Services.</p><a className="button button--red" href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer">Follow @247trucktyreservice <span aria-hidden="true">↗</span></a></div>
    </section>
  );
}
...
function AboutPreview() {
  return (
    <section className="about-preview">
      <div className="about-image"><Image src="/images/pack-03-workshop-truck.webp" alt="Black heavy truck inside a commercial tyre workshop" fill sizes="(max-width: 1180px) 100vw, 53vw" /><span>Regency Park<br />South Australia</span></div>
      <div className="about-copy">
        <p className="eyebrow"><span />Who we are</p>
        <div className="section-heading"><h2>Built around keeping trucks moving</h2></div>
        ...
        <Link className="button button--ghost-dark" href="/about">About the business <span aria-hidden="true">→</span></Link>
      </div>
    </section>
  );
}
...
function Process() {
  ...
  return (
    <section className="section process-section"><SectionHeading eyebrow="How it works" title="One call starts the response" /><div className="process-grid">{steps.map(([number, label]) => <div key={number}><span>{number}</span><h3>{label}</h3></div>)}</div></section>
  );
}

function FleetBand() {
  const benefits = [
    "Fleet and vehicle details held ready before a breakdown",
    "One-year membership, confirmed on activation",
    "Australia-wide enquiries welcome",
  ];
  return (
    <section className="fleet-band">
      ...
      <div className="fleet-copy">
        ...
        <div className="fleet-actions">
          <Link className="button button--red" href="/fleet-roadside-assistance">Register for the program <span aria-hidden="true">↗</span></Link>
          <a className="button button--ghost" href={PHONE_HREF}>Call the team</a>
        </div>
      </div>
    </section>
  );
...
function CustomerJourneys() {
  ...
  return (
    <section className="section journey-section" aria-labelledby="journey-title">
      ...
      <div className="journey-grid">{journeys.map(([eyebrow, title, label, href, modifier], index) => <article className={`journey-card ${modifier}`} key={title}><span>{String(index + 1).padStart(2, "0")}</span><p>{eyebrow}</p><h3>{title}</h3>{href.startsWith("tel:") ? <a href={href}>{label} <i aria-hidden="true">↗</i></a> : <Link href={href}>{label} <i aria-hidden="true">↗</i></Link>}</article>)}</div>
    </section>
  );
}
  ...
  return (
    <section className="location-section">
      <div className="location-copy">
        <p className="eyebrow"><span />Regency Park · Adelaide</p>
        <h2>Find 24/7 Truck Tyre Services</h2>
        <p>Truck tyre supply, fitting and roadside support from our Adelaide service location.</p>
        <dl className="location-facts">
          <div><dt>Location</dt><dd>Regency Park, South Australia</dd></div>
          <div><dt>Hours</dt><dd>Roadside assistance 24 hours, 7 days</dd></div>
        </dl>
        <div>
          <a className="button button--red" href="https://www.google.com/maps/dir/?api=1&destination=-34.85853674026474%2C138.57081007443298" target="_blank" rel="noopener noreferrer">Get directions <span aria-hidden="true">↗</span></a>
          <a className="button button--ghost" href={PHONE_HREF}>Call now</a>
        </div>
      </div>
      <div className="map-wrap"><iframe title="Google map showing 24/7 Truck Tyre Services in Regency Park" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3893.3547912856266!2d138.57081007443298!3d-34.85853674026474!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x6ab0c76a14442a27%3A0x827811fdc4c4f1da!2s24%2F7%20Truck%20tyre%20service!5e0!3m2!1sen!2sau!4v1788033590845!5m2!1sen!2sau" loading="lazy" referrerPolicy="strict-origin-when-cross-origin" /><span className="map-label">Regency Park · Adelaide</span></div>
    </section>
  );
}
  ...
  return (
    <form className="assistance-form" onSubmit={submit} aria-busy={submitting}>
      <div className="field-grid">
        ...
        <label><span>Phone *</span><input name="phone" type="tel" inputMode="tel" autoComplete="tel" required /></label>
        <label><span>Email</span><input name="email" type="email" inputMode="email" autoComplete="email" /></label>
      ...
      <div className="form-actions"><button className="button button--red" type="submit" disabled={submitting}>{submitting ? "Opening WhatsApp…" : "Request assistance"} <span aria-hidden="true">↗</span></button><p><strong>For urgent tyre assistance</strong><a href={PHONE_HREF}>Call {PHONE_DISPLAY}</a></p></div>
  ...
  return (
    <section className="contact-section" id="contact">
      <div className="contact-heading">
        <SectionHeading eyebrow="Request assistance" title="Tell us what your truck needs" intro="For an urgent roadside issue, calling is the fastest way to reach the team." />
        <a href={PHONE_HREF}><small>For urgent tyre assistance</small><span>Call {PHONE_DISPLAY}</span></a>
        <a className="contact-instagram" href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer">Follow @247trucktyreservice ↗</a>
      </div>
      <ContactForm />
    </section>
  );
...
export function HomePage() {
  return <SiteShell><StructuredData /><Hero /><TrustStrip /><CustomerJourneys /><ServicesSection /><FleetBand /><WhyUs /><Process /><FranchiseBand /><ServiceAreas /><AboutPreview /><Gallery /><EmergencyStrip /><FAQ /><ContactSection /><Location /></SiteShell>;
}
...
export function ContactPage() {
  return <SiteShell><section className="page-masthead"><p className="eyebrow"><span />Contact the team</p><h1>Truck tyre help starts here</h1><p>Send your service details through WhatsApp, or call now for urgent 24/7 tyre assistance.</p></section><ContactSection /><Location /><FAQ /></SiteShell>;
}
